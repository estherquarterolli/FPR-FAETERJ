...
typedef struct {
    ...
    float idd;
    float doutores;
    float mestres; // principais dados que serão processados e mostrados depois.
...
int main() {
    ...
    if (arqCheck == NULL) {
        ...
        if (arqCheck == NULL) {
            printf("Erro ao criar o arquivo EnadeCursos.txt\n");
            return 1;
        }
        fclose(arqCheck);
    } else {
...
// Função para ler o arquivo e armazenar no vetor de structs
int lerArq(char nomeArquivo[], Tcursos curso[], int *totCursos) {
...
// Função para adicionar um novo curso e salvar no arquivo
int addCurso(char nomeArquivo[], Tcursos curso[], int *totCursos) {
...
// Calcula CPC continuo e CPC faixa do curso
void calcularCPC(Tcursos *curso) {
...
// Converte CPC continuo para faixa (1-5)
int converterCPCFaixa(float cpc) {
    if (cpc < 0.945f)
        return 1;
    else if (cpc < 1.945f)
...
// Função para gerar relatório dos cursos e IGC
void gerarRel(Tcursos curso[], int totCursos) {
    int i, faixa;
    int countFaixa[6] = {0, 0, 0, 0, 0, 0}; // índice 1 a 5 usados

    // Exibir dados de cada curso
    ...
    // Para cada CPC faixa, mostrar os cursos
